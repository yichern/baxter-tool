...
from simulation.envs import rotations, robot_env, utils
...
class BaxterEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        super(BaxterEnv, self).__init__(
            model_path=model_path, n_substeps=n_substeps, n_actions=9,
    ...
    def _step_callback(self):
        if self.block_gripper:
            self.sim.data.set_joint_qpos('l_gripper_l_finger_joint', 0.)
            self.sim.data.set_joint_qpos('l_gripper_r_finger_joint', 0.)
            self.sim.forward()

    def _set_action(self, action):
        assert action.shape == (9,)
        action = action.copy()  # ensure that we don't change the action outside of this scope
        pos_ctrl, gripper_ctrl = action[0:7], action[7:9]
    ...
    def _render_callback(self):
        ...
        site_id = self.sim.model.site_name2id('box')
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            object_qpos = self.sim.data.get_joint_qpos('box:joint')
            assert object_qpos.shape == (7,)
            object_qpos[:2] = object_xpos
            self.sim.data.set_joint_qpos('box:joint', object_qpos)
